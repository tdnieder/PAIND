...
*       code with unkonwn author at iHomeLab. This software extends
 ...
 ***********************************************/
//System declarations
#define TICK        10          //System Tick in ms  !!!!when change, recalculate Turn-Off Time!!!!
#define BAUDRATE    115200      //Hardware Serial Baudrate (con. to PC)

//PIN declarations
#define BATTERY     0           //battery balance on Analog 0
...
#define ROLLATOR_UNPLUGGED	1	//If Rollator is unplugged and tests are executed
...
void loop()
{
	...
	if(DEBUG_ENABLED)
		Serial.println(String(x) + "/"+ String(y) + "\t" + String(lValue) + "/" +String(rValue));
	...
	*/
	...
	{
		/*
		//check bluetooth buffer for new target values
		readBluetooth();
		*/
...
/**
 * Sends initializing commands to bluetooth modul to set it
 * up for connection with remote controller. 
 * <p>
...
void readBluetooth()
{
  ...
  left.w_k = (recvString.substring(1, indexOfDelimiter)).toInt();
  right.w_k = (recvString.substring(indexOfDelimiter + 1, indexOfEnd)).toInt();
  //if rollator active, reset watchdog
  if((left.w_k || right.w_k))
	watchCounter = 0;
  if(DEBUG_ENABLED)
	Serial.println("Joystick Value:" + String(left.w_k) + " / " + String(right.w_k));
}
